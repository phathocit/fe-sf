  ...
  if (!stall) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-4 bg-slate-50">
        ...
        <h2 className="text-3xl md:text-5xl font-black text-slate-800 mb-4 italic tracking-tighter">Oops! Lỗi 404</h2>
        <p className="text-slate-500 font-bold mb-8">Không tìm thấy gian hàng này, có thể quán đã dời đi hoặc đóng cửa.</p>
        <Link to="/" className="cursor-pointer inline-flex items-center gap-2 bg-orange-600 text-white px-8 py-4 rounded-full font-black text-sm uppercase tracking-widest hover:bg-orange-500 hover:-translate-y-1 shadow-xl shadow-orange-500/30 transition-all active:scale-95 group">
    ...
    <div className="min-h-[calc(100vh-64px)] bg-gray-50 pb-20">
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 md:p-10 max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-white drop-shadow-lg mb-2">{stall.name}</h1>
              <div className="flex flex-col gap-1.5 mt-3">
                <p className="text-white/90 text-sm md:text-base font-medium max-w-2xl flex items-start sm:items-center gap-2 m-0 bg-black/20 w-fit px-3 py-1.5 rounded-full backdrop-blur-sm">
                   <MapPin size={16} className="text-orange-400 shrink-0 mt-0.5 sm:mt-0" /> {stall.address}
                </p>
                <p className="text-white/90 text-sm md:text-base font-medium max-w-2xl flex items-center gap-2 m-0 bg-black/20 w-fit px-3 py-1.5 rounded-full backdrop-blur-sm">
                   <Clock size={16} className="text-yellow-400 shrink-0" /> Mở cửa: {stall.operatingHours}
                </p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row items-stretch gap-3 mt-4 md:mt-0 w-full md:w-auto h-full">
               <Link to={`/map`} className="cursor-pointer bg-linear-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white shadow-lg shadow-orange-500/30 px-5 py-4 rounded-2xl flex items-center justify-center sm:justify-start gap-3 transition-all active:scale-95 border border-orange-400/50 group flex-1">
                 ...
                 <div className="text-left flex flex-col justify-center">
                   <div className="text-2xl font-black leading-none tracking-tight">BẢN ĐỒ</div>
                   <div className="text-white/80 text-[10px] sm:text-xs font-medium uppercase mt-1">Định vị quán</div>
                 </div>
               </Link>
               ...
               <div className="bg-white/10 backdrop-blur-xl border border-white/20 px-5 py-4 rounded-2xl flex items-center justify-center sm:justify-start gap-3 text-white flex-1">
                  ...
                  <div className="text-left flex flex-col justify-center">
                     <div className="text-3xl font-black leading-none">{stall.rating}</div>
                     <div className="text-white/60 text-[10px] sm:text-xs font-medium uppercase mt-1">Đánh giá chung</div>
                  </div>
               </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 mt-[-2rem] relative z-10">
        <div className="bg-white rounded-3xl p-8 shadow-xl shadow-gray-200/50 border border-gray-100 mb-10">
           ...
           <p className="text-gray-600 leading-relaxed text-lg">{stall.description}</p>
        </div>

        <h3 className="text-3xl font-black text-gray-900 mb-8 flex items-center gap-3">
           Thực Đơn
           <span className="bg-orange-100 text-orange-600 text-sm font-bold px-3 py-1 rounded-full">{stall.menu.length} món</span>
        </h3>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {stall.menu.map((item) => (
            <div key={item.id} className="bg-white rounded-2xl flex overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:border-orange-200 transition-all duration-300 group">
              ...
              <div className="p-5 flex-1 flex flex-col justify-center">
                <h4 className="font-bold text-lg text-gray-800 mb-1 group-hover:text-orange-600 transition-colors">{item.name}</h4>
                {item.description && <p className="text-sm text-gray-500 line-clamp-2 mb-3 leading-relaxed">{item.description}</p>}
                <div className="mt-auto font-black text-orange-600 text-xl flex items-end">
                   {item.price.toLocaleString('vi-VN')} <span className="text-sm font-bold ml-1 text-gray-400 mb-1">VNĐ</span>
                </div>
              </div>
            </div>
          ))}
