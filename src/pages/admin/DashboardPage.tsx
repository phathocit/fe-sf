                ...
                <div className="max-w-7xl mx-auto animate-in fade-in duration-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 mb-12">
                        {cards.map((card, index) => (
                            ...
                            >
                                <div className={`absolute -right-4 -top-4 w-24 h-24 bg-linear-to-br ${card.color} opacity-0 group-hover:opacity-10 rounded-full transition-all duration-700 blur-2xl`}></div>
                                
                                <div className={`${card.bg} w-16 h-16 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500`}>
                                    <card.icon size={32} className={card.textColor} />
                                </div>
                                
                                <h3 className="text-slate-500 font-bold text-sm uppercase tracking-widest mb-2">{card.title}</h3>
